...
- [x] 修正重复 ID 统计，让多个 `data_source` 指针按逻辑路径计数，并补充当前 8 个合法重复 QID 到允许列表。
...
- [x] 新增按来源轮转的候选调度、0 请求下轮预览、来源结果统计和近轮效率摘要，减少同一来源在小预算下的集中消耗。
- [x] 新增 `getScanState` / `getStats` / `getNextSchedule` 静态接口，并在页面顶部展示扫描诊断面板。
...
- 观察 `data/scan_state.json` 的 `candidate_count`、`exhausted`、`candidate_source_summary`、`source_cooldowns`、`source_outcome_counts`、`max_sources_per_node` 和节点的 `source_checked` / `source_no_children`，判断增长慢是候选不足、来源限流，还是当前来源已查空。
