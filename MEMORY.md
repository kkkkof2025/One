...
- GitHub Pages 静态接口镜像写入 `data/api/`。
...
- 静态页面 `index.html` 直接读取 `data/root.json`，再按需加载 `data/nodes/*.json`。
- `index.html` 优先读取 `data/api/` 下的接口式 JSON，旧部署缺少接口文件时回退到 `data/root.json` 和 `data/nodes/*.json`。
...
- `data/validation_allowlist.json` 记录人工确认过的合法重复 QID；当前允许 `Q79925`（暗物质）和 `Q18343`（暗能量）同时出现在“宇宙”和“可观测宇宙”路径中。
...
- `scripts/grow_json.py` 会生成 `data/api/` 静态接口镜像：`root.json` 返回根节点，`nodes/Q....json` 返回节点，`children/nodes/Q....json` 返回子节点摘要，`getEndNode.json` 返回终止节点清单。
- 页面读取数据时优先使用 `data/api/`，缺少接口文件时回退到旧的 `data/root.json` 和 `data/nodes/*.json`，以便旧部署仍可打开。
